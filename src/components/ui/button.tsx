...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-medium ring-offset-background transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 relative overflow-hidden",
  {
    variants: {
      variant: {
        default: "bg-gradient-ocean text-primary-foreground shadow-ocean hover:scale-105 hover:shadow-premium",
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90 hover:scale-105",
        outline:
          "border-2 border-primary/30 bg-background hover:bg-primary/5 hover:border-primary/50 hover:scale-105",
        secondary:
          "bg-gradient-sunset text-secondary-foreground hover:scale-105 shadow-coral",
        ghost: "hover:bg-accent hover:text-accent-foreground hover:scale-105",
        link: "text-primary underline-offset-4 hover:underline",
        premium: "btn-premium",
        glass: "glass-card hover:scale-105 text-primary font-semibold",
      },
      size: {
        default: "h-12 px-6 py-3 touch-target",
        sm: "h-11 rounded-lg px-4 py-2 touch-target",
        lg: "h-16 rounded-xl px-10 py-4 text-lg touch-target",
        icon: "h-12 w-12 touch-target",
        xl: "h-20 rounded-2xl px-12 py-6 text-xl touch-target",
      },
    },
    defaultVariants: {
      variant: "default",
